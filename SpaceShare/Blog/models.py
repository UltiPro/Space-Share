...
from .regexes import (
    regex_validator_email,
    regex_validator_login,
...
class Author(models.Model):
    ...
    description = models.TextField(
        validators=[MinLengthValidator(15), MaxLengthValidator(2000)],
        default="Information about author.",
    )
...
class User(models.Model):
    ...
    login = models.CharField(
        unique=True, max_length=15, validators=[regex_validator_login]
    )
    password = models.CharField(max_length=100, validators=[regex_validator_password])
    nickname = models.CharField(
        unique=True, max_length=15, validators=[regex_validator_nickname]
    )
    email = models.EmailField(unique=True, validators=[regex_validator_email])
    image = models.ImageField(upload_to="users", null=True, default="users/default.png")
    description = models.TextField(
        validators=[MinLengthValidator(15), MaxLengthValidator(2000)],
        default="Information about user.",
        null=True,
    )
