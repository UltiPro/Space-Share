...
class NewsletterForm(forms.ModelForm):
    ...
    def send_email(self):
        ...
        html_mail = render_to_string(
            "Blog/emails/newsletter.html",
            {"name": cleaned_data.get("name"), "surname": cleaned_data.get("surname")},
        )
        try:
            send_mail(
...
class UserRegisterForm(forms.ModelForm):
    ...
    class Meta:
        model = UserModel
        exclude = ["slug", "image", "description", "is_active"]
        widgets = {
    ...
    def clean(self):
        ...
        if password != c_password:
            raise forms.ValidationError({"c_password": "Passwords do not match!"})
        try:
...
class UserLoginForm(forms.Form):
    ...
    password = forms.CharField(
        label="Password",
        validators=[regex_validator_password],
        widget=forms.PasswordInput(
            attrs={
                "class": "form-control border border-secondary",
                "placeholder": "Confirm Password",
...
class ChangeEmailForm(forms.ModelForm):
    old_email = forms.EmailField(
        label="New E-mail",
        validators=[regex_validator_email],
    ...
    def clean(self):
        ...
        if cleaned_data.get("old_email") == cleaned_data.get("email"):
            raise forms.ValidationError({"email": "The old email cannot be new email."})
